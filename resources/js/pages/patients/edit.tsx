...
import { PatientEditModalProps } from './types';

export function PatientEditModal({ open, onOpenChange, patient, ...props }: PatientEditModalProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (patient) {
            put(`/patients/${patient.id}`, {
                ...
                onError: () => {
                    toast.error(t('Failed to update patient'), { description: t('Please check the form and try again') });
                },
            });
        }
    };
        ...
        <Modal open={open} onOpenChange={handleClose}>
            <ModalContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
                <form onSubmit={handleSubmit}>
                    <ModalHeader>
                        <ModalTitle>{t('Edit Patient')}</ModalTitle>
                    </ModalHeader>
