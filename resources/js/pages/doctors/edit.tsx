...
import { DoctorEditModalProps } from './types';

export function DoctorEditModal({ open, onOpenChange, doctor, ...props }: DoctorEditModalProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (doctor) {
            put(`/doctors/${doctor.id}`, {
                ...
                onError: () => {
                    toast.error(t('Failed to update doctor'), { description: t('Please check the form and try again') });
                },
            });
        }
    };
        ...
        <Modal open={open} onOpenChange={handleClose}>
            <ModalContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
                <form onSubmit={handleSubmit}>
                    <ModalHeader>
                        <ModalTitle>{t('Edit Doctor')}</ModalTitle>
                    </ModalHeader>
